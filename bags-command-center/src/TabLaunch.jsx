...
export default function TabLaunch() {
  ...
  const [form, setForm] = useState({ name: '', ticker: '', description: '', twitter: '', website: '', imagePreview: null, feeShare: 1 })
  ...
  const handleImage = e => {
    ...
    reader.onload = ev => up({ imagePreview: ev.target.result })
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 28 }}>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <label style={{ border: '2px dashed rgba(249,115,22,0.3)', borderRadius: 14, padding: 40, textAlign: 'center', cursor: 'pointer', display: 'block' }}>
              {form.imagePreview ? <img src={form.imagePreview} alt="" style={{ width: 120, height: 120, borderRadius: '50%', objectFit: 'cover', margin: '0 auto', display: 'block' }} /> : <div><div style={{ fontSize: 40 }}>🖼️</div><div style={{ color: '#475569', marginTop: 8 }}>Click to upload image</div></div>}
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <p style={{ margin: 0, color: '#475569', fontSize: 14 }}>Set your creator fee — you earn this % from every trade on Bags.</p>
            ...
            <div style={{ padding: 14, borderRadius: 10, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', fontSize: 13, color: '#10b981' }}>
              💡 At {form.feeShare}% — 100 SOL volume = {(100 * form.feeShare / 100).toFixed(2)} SOL earned
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 16, borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
              {form.imagePreview ? <img src={form.imagePreview} alt="" style={{ width: 60, height: 60, borderRadius: '50%', objectFit: 'cover' }} /> : <div style={{ width: 60, height: 60, borderRadius: '50%', background: 'linear-gradient(135deg,#f97316,#ef4444)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 800 }}>{form.name.charAt(0)}</div>}
              <div>
